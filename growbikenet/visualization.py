...
def generate_plots(edges_ordered, nodes, ordering, with_existing_bike_network):
    ...
    for framenum in tqdm(
        ...
        desc=("{:<"+str(constants._PROGRESS_BAR_DESC_LENGTH)+"}").format("Generating plots"),
        leave=True,
        unit="plot",
...
def generate_plot_one_step(edges_ordered, nodes, ordering, with_existing_bike_network, framenum, point_data, edges):
    """Plot frame of one step in a growing bicycle network .

    Result is a png file saved into `settings.export_path['plots']`.
    ...
    edges_ordered.to_crs(settings.viz["crs"], inplace=True)
    nodes.to_crs(settings.viz["crs"], inplace=True)
    point_data.to_crs(settings.viz["crs"], inplace=True)
    edges.to_crs(settings.viz["crs"], inplace=True)
    ...
    edges.plot(
            ax=ax, color="#AAAAAA", lw=0.25, zorder=0
    )
    ...
    point_data.plot(
        ax=ax, color="red", zorder=1, alpha=0.33,
    )
    ...
    fig.savefig(settings.export_path['plots']+f"ordering_{ordering}/{plot_id}single.png", dpi=settings.viz['dpi'])
    figs[framenum] = fig
    plt.close()
