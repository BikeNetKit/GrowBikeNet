...
def growbikenet(
    ...
    if ranking != "betweenness_centrality" and ranking != "closeness_centrality":
        raise ValueError(
            "ranking must be 'betweenness_centrality' or 'closeness_centrality'"
        )
    ...
    if (
        seed_point_type == "grid"
        and type(seed_point_grid_spacing) == int
        and seed_point_grid_spacing <= 0
    ):
    ...
    nodes, edges, g_undir = prepare_network(
        city_name, proj_crs, network_type="all_public"
    )
    ...
    if existing_network_spacing:
        ...
        nodes_exnw, edges_exnw, g_undir_exnw = prepare_network(
            city_name, proj_crs, custom_filter=cf
        )
        g_undir = nx.compose(
            g_undir_exnw, g_undir
        )  # Merge to be sure we have everything from both
    ...
    if existing_network_spacing:
        ...
        seed_points_exnw = get_existing_network_seed_points(
            nodes_exnw, existing_network_spacing
        )
        ...
        buffer_seed_points_exnw = gpd.GeoDataFrame(
            seed_points_exnw.buffer(existing_network_spacing)
        )
        buffer_seed_points_exnw = buffer_seed_points_exnw.rename(
            columns={0: "geometry"}
        ).set_geometry(
            "geometry"
        ...
        seed_points_snapped = seed_points_snapped.overlay(
            buffer_seed_points_exnw, how="difference"
        )
        ...
        seed_points_snapped.loc[seed_points_snapped["osmid_1"].isnull(), "osmid_1"] = (
            seed_points_snapped["osmid_2"]
        )  # _1 comes from one side, _2 from the other. One has NaNs, the other too. https://stackoverflow.com/a/60132614
        seed_points_snapped.drop(
            ["y", "x", "street_count", "highway", "osmid_2"], axis=1, inplace=True
        )
