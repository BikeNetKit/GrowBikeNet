...
def growbikenet(
    ...
    The edges form a subnetwork of a city's street network, interpreted as a growing bicycle network following [1]_. By default, growth is from scratch, but the existing bicycle network can also be used as a starting point [2]_.
    ...
    ordering : str ('betweenness' | 'closeness' | 'random'), default 'betweenness'
    ...
    seed_point_grid_spacing : 'auto' | int, default 'auto'
        ...
        These values ensure that any point in the city is always within 500m of the network (under perfect conditions). For the explanation of case 1707 see [1]_.
    ...
    export_data : bool, default True
        If set to True, data is saved to a file. The filename is ``[slug]-growbikenet-[ordering]-from_scratch|from_bikenw-[seed_point_type]-with_overlaps|no_overlaps.[settings.export_file_format]``, depending on the respective parameters, and where ``[slug]`` is a string id made out of `city_query` (or `city_id` if set).
